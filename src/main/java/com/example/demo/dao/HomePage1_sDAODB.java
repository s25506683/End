...
//  }
 ...
 public List<HomePage1_s> findStudentCourse(int std_id) {
    return this.jdbcTemplate.query( "select c.cs_id, c.cs_name, c.cs_photo, t.teacher_name,cs.std_id from class c inner join class_student cs on c.cs_id = cs.cs_id inner join class_teacher ct on c.cs_id = ct.cs_id inner join teacher t on ct.teacher_id = t. teacher_id where cs.std_id = ? order by c.cs_id", new Object[]{std_id}, new HomePage1_sMapper());
  }

 public List<HomePage1_s> findAllCourse() {
     return this.jdbcTemplate.query( "select distinct c.cs_id, c.cs_name, c.cs_photo, t.teacher_name from class c inner join class_student cs on c.cs_id = cs.cs_id inner join class_teacher ct on c.cs_id = ct.cs_id inner join teacher t on ct.teacher_id = t. teacher_id order by c.cs_id", new HomePage1_sMapper2());
 }
